...
options = SimpleNamespace(
    ...
    ##
    # Whether to lower case aliases or not
    lower_aliases = False,
...
def remove_punc_label_laurel(label):
    label = label.replace(".", "")
    label = label.replace("!", "")
    label = label.replace("?", "")
    return label

def remove_punc_title_laurel(title):
    title = title.replace(".", "~*~")
    title = title.replace("!", "~**~")
    title = title.replace("?", "~***~")
    title = title.replace(":", "~****~")
...
def undo_remove_punc_title_laurel(title):
    title = title.replace("~*~", ".")
    title = title.replace("~**~", "!")
    title = title.replace("~***~", "?")
    title = title.replace("~****~", ":")
...
class Extractor(object):
    ...
    def write_output_sent(self, out, bold_aliases, acronym_aliases, all_aliases):
        """
    ...
    def extract(self, out1, out2):
        ...
        text = " ".join(compact(self.clean(text)))
        all_aliases = []
        sent_idx = 0
        # the etc. messes with the sentence tokenization
        text = text.replace("etc.", "etc")
        # this is just empty parenthesis to remove
        text = text.replace("()", "")
        # this is bacause the tokenization doesn't work if a <a tag is attached to a dash e.g. "anarcho-<a href="hedonist">hedonist"
        text = text.replace("-<a", " <a")
        text = text.replace("<a", " <a")
        text = text.replace("</a>", "</a> ")
        # sometimes the sent_tokenizer splits in between html tags; so, if there is an error, we just merge it with the next sentence
        # print("TEXT", text)
        sentences = sent_tokenize(text)
        st_idx = 0
        end_idx = 1
        while st_idx < len(sentences):
            sent = " ".join(sentences[st_idx:end_idx])
            ...
            # alias_spans = list(map(lambda x: f"{x[0]}:{x[1]}", alias_spans))
            json_data = {
            ...
            # print("JSON_DATA", self.title)
            # print(json.dumps(json_data, indent=4))
            sent_idx += 1
            st_idx = end_idx
            end_idx = st_idx + 1
            all_aliases.append(json_data)
        self.write_output_sent(out1, bold_aliases, acronym_aliases, all_aliases)
    ...
    def extract_aliases(self, sent):
        ...
        cur = 0
        if len(tags) == 0:
            return aliases, page_titles, alias_spans, sent
        # print("START SENT", sent)
        for t in tags:
            # print("+++++++++++++++++")
            # print("RES PRE")
            # print(word_tokenize(res))
            st_idx = t.sourcepos
            # print("CUR", cur, "START", st_idx, "RAW TAG", t, t.get("href"))
            end_idx = sent.find(end_tag, st_idx) + len(end_tag)
            # print("TO UPDATE")
            # print(sent[cur:st_idx])
            res += sent[cur:st_idx]
            # print("RES POST")
            # print(word_tokenize(res))
            cur_w = len(word_tokenize(res))
            ...
            # print("TITLE", title, "ALIAS ARR 2", al_arr)
            if 'href' in res:
                print("***********************************")
                print("TAG", t, '--', t.get("href"), '--', t.sourcepos)
                print("SENT", sent)
                print("RES", word_tokenize(res))
                print("TITLE", title, "ALIAS", al_arr)
                print("CUR_W", cur_w)
                print("SPAN", [cur_w, cur_w+len(al_arr)])
            else:
                page_titles.append(title)
                al_arr_str = " ".join(al_arr)
                if options.lower_aliases:
                    al_arr_str = al_arr_str.lower()
                aliases.append(al_arr_str)
                alias_spans.append([cur_w, cur_w+len(al_arr)])
            cur = end_idx
        res += sent[cur:]
        return aliases, page_titles, alias_spans, res

    # LAUREL
    def extract_acronyms(self, sentence_str):
        """
        Finds acronyms for the page title in parenthesis. Modified from acronym code in acronym_utils.py
        """
        ...
        while left < len(sentence_str) and right < len(sentence_str):
            if sentence_str[left] == "(":
                ...
                words = sentence_str[left+1:right].replace("'", "").split()
                ...
                if (len(words) == 1 and len(words[0]) >= 3) or comma_flag:
                    ...
                    if sum(map(str.isupper, acronym)) / len(acronym) >= 2/3:
                        matches = greedy_match(self.title, acronym)
                        if matches:
    ...
    def wiki2text(self, text):
        ...
        for sent in sentences:
            new_aliases = bold_italic.findall(sent)
            sent = bold_italic.sub(r'\1', sent)
            new_aliases.extend(bold.findall(sent))
            # Remove linked aliases if exist
            temp = []
            for al in new_aliases:
                if len(al) <= 0 or any([w in al for w in not_allowed]) or al.isnumeric():
                    continue
                temp.append(al)
            new_aliases = temp
            if options.lower_aliases:
                new_aliases = [al.lower() for al in new_aliases]
            bold_aliases.extend(new_aliases)
        # print("FOUND ALIASES", bold_aliases, "FOR", self.title)

        # LAUREL: try to extract acronyms from first sentences
        found_acronyms = set()
        for sent in sentences:
...
def makeExternalLink(url, anchor):
    ...
    if options.keepLinks:
        ...
        return '<a href="%s">%s</a>' % (remove_punc_title_laurel(quote(url.encode('utf-8'))), remove_punc_label_laurel(anchor))
    else:
        return anchor
...
def extract_process(opts, i, jobs_queue, output_queue):
    ...
    while True:
        ...
        if job:
            ...
            output_queue.put((page_num, text1, text2))
            out1.truncate(0)
            out1.seek(0)
            out2.truncate(0)
            out2.seek(0)
...
def reduce_process(opts, output_queue, spool_length,
    ...
    if out_file2:
        nextFile2 = NextFile(out_file2)
        output2 = OutputSplitter(nextFile2, file_size, file_compress)
    else:
        output2 = sys.stdout if PY2 else sys.stdout.buffer
        if file_compress:
            logging.warning("writing to stdout, so no output compression (use an external tool)")
    ...
    while True:
        if next_page in spool:
            out1, out2 = spool.pop(next_page)
            output.write(out1.encode('utf-8'))
            output2.write(out2.encode('utf-8'))
            next_page += 1
            ...
            if not pair:
                break
            page_num, text1, text2 = pair
            spool[page_num] = text1, text2
            # tell mapper our load:
            spool_length.value = len(spool)
...
def main():
    ...
    groupO.add_argument("-o", "--output", default="sentences",
                        help="directory for extracted files (or '-' for dumping to stdout)")
    groupO.add_argument("-o2", "--output2", default="pageids",
                        help="directory for extracted pageid mappings (or '-' for dumping to stdout)")
    ...
    groupP.add_argument("--lower_aliases", action='store_true',
                        help="use lower casing of aliases when recording")
    ...
    options.keepLinks = args.no_links
    ...
    options.lower_aliases = args.lower_aliases
    ...
    if output_path2 != '-' and not os.path.isdir(output_path2):
        try:
            os.makedirs(output_path2)
        except:
            logging.error('Could not create: %s', output_path2)
            return
