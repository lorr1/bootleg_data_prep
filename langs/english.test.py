...
assert isinstance(language.stem('some text'), str)
pos_tagged = language.pos_tag(['highway', 'to', 'hell'])
assert isinstance(pos_tagged, list)
assert isinstance(pos_tagged[0], tuple)
bigrams = language.bigrams(['bla', 'bla', 'bla'])
bigrams_list = list(bigrams)
assert isinstance(bigrams_list, list)
assert isinstance(bigrams_list[0], tuple)
