...
from urllib.parse import quote as urlencode, unquote, quote
...
def makeInternalLink(title, label):
    colon = title.find(':')
    if colon > 0 and (title[:colon].lower() not in acceptedNamespaces and title[:colon].lower() in standardNamespaces):
        return ''
    if colon == 0:
        # drop also :File:
