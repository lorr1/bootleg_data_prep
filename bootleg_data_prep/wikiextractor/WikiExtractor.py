...
def extract_process(jobs_queue, output_queue, html_safe):
    ...
    while True:
        ...
        if job:
            ...
            out2 = StringIO()  # memory buffer
            Extractor(*job[:-1]).extract(out, out2, html_safe)  # (id, urlbase, title, page)
            text = out.getvalue()
            text2 = out2.getvalue()
            output_queue.put((job[-1], text, text2))  # (ordinal, extracted_text)
            out.close()
            out2.close()
        else:
            break
...
def reduce_process(output_queue, output, output2):
    ...
    while True:
        if next_ordinal in ordering_buffer:
            text1, text2 = ordering_buffer.pop(next_ordinal)
            output.write(text1)
            output2.write(text2)
            next_ordinal += 1
            # progress report
            ...
            if not pair:
                break
            ordinal, text1, text2 = pair
            ordering_buffer[ordinal] = text1, text2
...
def main():
    ...
    groupP.add_argument("-l", "--links", action="store_true", default=True,
                        help="preserve links")
    ...
    if args.article:
        ...
        with open(input_file, 'r', encoding='utf-8') as file:
            ...
            if m:
                ...
                urlbase = ''
            Extractor(id, revid, urlbase, title, [page]).extract(sys.stdout, sys.stdout)
        return
